      ...
      <table class="table table-zebra table-pin-rows text-center">
        ...
        <tbody v-auto-animate>
          <tr v-for="item in props.records.records" :key="item.SID">
            <td class="font-bold">
              <span
                :style="'background-color: ' + submitStatusColor[item.Result]"
            ...
            <td class="hidden md:table-cell">{{ item.UseTime }} ms</td>
            <td class="hidden lg:table-cell">
              {{ Math.ceil(item.UseMemory / 1024 / 1024) }} MB
            </td>
            <td class="[&_span]:font-bold">
              <span
                class="text-blue-500 tooltip hover:text-blue-400 cursor-pointer"
                data-tip="查看代码"
                @click="
                  $router.push({
                    name: 'Record',
                ...
                "
                v-if="
                  userDataStore.PermissionMap &
                    constValStore.SourceBorwserBit ||
                  userDataStore.UID === item.UID
                "
              >
...
const maxPage = computed(() =>
  Math.ceil(props.records.count / props.records.limit)
);
</script>
