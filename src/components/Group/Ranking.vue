<template>
  <div class="card bg-white Border shadow-lg overflow-x-auto rounded-t-none" style="max-height: calc(100vh - 124px - 48px)">
    <table class="table table-zebra table-pin-rows table-pin-cols table-fixed text-center">
      <thead>
        <tr>
          ...
          <th class="w-16">Solved</th>
          <!-- <th class="w-20">Penalty</th> -->
          <th v-for="(item, index1) in props.problems.length" :key="index1" class="w-8">
...
let ranking = reactive<ContestRankingType>({
  ...
  get() {
    ...
    _getProblemListRanking(params, props.problemList.LID)
      .then((data: any) => {
  ...
  getPionners() {
    for (let i = 0; i < props.problems.length; i++) {
      let pioneer = -1, minTime = props.problemList.EndTime - props.problemList.BeginTime;
...
onMounted(() => {
  if (props.problemList.LID == 0) {
    props.problemList.LID = route.params.LID as unknown as number;
  }
  ranking.get();
