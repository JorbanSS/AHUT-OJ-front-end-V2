      ...
      <div v-for="discussion in discussions.discussions" :key="discussion.SID">
        <div class="card border shadow-lg p-6 bg-white mb-4">
            ...
            <div class="flex flex-row items-center">
              <img :src="getHeadURL(discussion.HeadURL)" alt="Avatar" class="w-12 h-12 rounded-full mr-4 cursor-pointer"
                @click="
                  $router.push({
                    name: 'User',
                    params: { UID: discussion.UID },
                  })
                  " />
              <div>
                <div class="flex space-x-2">
                  <div class="font-bold cursor-pointer" @click="
                    $router.push({
                      name: 'User',
                      params: { UID: discussion.UID },
                    })
                ...
                <div class="text-gray-500 text-sm">
                  {{ ConvertTools.PrintTime(discussion.CreateTime, 1, 1) }}
                </div>
              </div>
            </div>
            <div class="flex space-x-2">
              ...
              <button :style="{
                backgroundColor: discussion.IsFavorite ? '#eb4868' : '',
                color: discussion.IsFavorite ? 'white' : '',
              }" class="btn flex items-center space-x-1" @click="
                discussions.giveThumbsUp(
                  discussion.SID,
              ...
              <button class="btn flex items-center space-x-1">
                ...
                <span>
                  {{ discussion.Data == null ? 0 : discussion.Data.length }}
                </span>
              </button>
            </div>
          </div>
          ...
          <div class="chat chat-start">
            <div class="chat-image avatar">
              <div class="w-12 rounded-full">
                <img alt="Avatar" :src="getHeadURL(userDataStore.HeadURL)" class="cursor-pointer" @click="
                  $router.push({
                    name: 'User',
            ...
            <div class="chat-header flex items-baseline gap-1" :class="{
              'flex-row-reverse': userDataStore.UID == discussion.UID,
            }">
              <span class="cursor-pointer font-bold" @click="
                $router.push({
                  name: 'User',
                  params: { UID: userDataStore.UID },
                })
          ...
          <button class="btn btn-neutral w-fit ml-auto mt-1"
            @click="discussions.comment(discussion.SID, discussion.NewComment)">
            发表评论
          </button>
      ...
      <div class="modal-box max-w-5xl">
        ...
        <div class="modal-action">
          <form method="dialog">
            ...
            <button class="btn btn-neutral" @click="newDiscussion.add()">
              发表话题
            </button>
          </form>
        </div>
    ...
    <dialog id="discussionModal" class="modal">
      <div class="modal-box max-w-5xl">
        ...
        <div class="modal-action">
          <form method="dialog">
            <button class="btn mr-2">取消</button>
            <button class="btn btn-neutral" @click="editDiscussion.edit()">
              提交编辑
            </button>
          </form>
        </div>
...
let editDiscussion = reactive<NewDiscussionType>({
  ...
  openEditModal(SID: number) {
    const foundDiscussion = discussions.discussions.find(
      (item) => item.SID === SID
    );
    if (foundDiscussion) {
      this.SID = SID;
...
let discussions = reactive<DiscussionsType>({
  ...
  get() {
    ...
    _getDiscussions(params).then((data: any) => {
  ...
  giveThumbsUp(SID: number, IsFavorite: number) {
    ...
    _giveThumbsUp(params).then(() => {
      discussions.discussions.forEach((item) => {
        if (item.SID == SID) {
          item.IsFavorite = !item.IsFavorite;
          item.FavoriteCount = IsFavorite
