    ...
    >
      ...
      <MainContainer>
        <Col v-auto-animate>
          <Col class="gap-0">
            <Card
              class="rounded-b-none p-6"
              v-if="contest.CID || problemList.LID"
            >
              <div class="flex flex-col gap-2">
                <div class="text-lg flex gap-2 justify-between">
                  <div
                    class="space-x-2 cursor-pointer hover:text-blue-500"
                    @click="
                      $router.push({
                  ...
                  <button
                    ...
                    :disabled="!userDataStore.isLogin"
                  ...
                  <progress
                    ...
                    :value="
                      ConvertTools.Percentage(
                        Math.min(
                          contest.Duration,
                          contest.TimeNow - contest.BeginTime
                        ),
                        contest.Duration
                      )
                    "
                ...
                <div
                  ...
                  :style="`grid-template-columns: repeat(auto-fit, ${
                    (Math.min(leftWidth, 1222) - 90) /
                      Math.max(
                        Math.floor((Math.min(leftWidth, 1222) - 90) / 180),
                        2
                      ) -
                    9
                  }px)`"
                >
                  ...
                  >
                    <button
                      tabindex="0"
                      role="button"
                      class="btn w-full justify-start flex-nowrap group-hover/dropdown"
                    ...
                    >
                      <div class="card-body">
                        ...
                        <p>
                          AC 率
                          <progress
                            class="progress progress-success w-20 mx-2"
                            :value="
                              ConvertTools.Percentage(
                                item.ACNum,
                                item.SubmitNum
            ...
            <Card
              class="p-6"
              :class="{ 'rounded-t-none': contest.CID || problemList.LID }"
            >
              ...
              <div
                class="flex flex-row gap-2 [&_div]:gap-1 flex-wrap"
                v-auto-animate
              >
                <div
                  class="flex badge badge-neutral badge-lg rounded-lg h-8 whitespace-nowrap"
                >
                ...
                <div
                  class="flex badge badge-neutral badge-lg rounded-lg h-8 whitespace-nowrap cursor-pointer"
                  @click="showLabels = !showLabels"
                  v-if="problem.Label"
                >
                  <tag-one theme="outline" />
                  <span class="select-none">题目标签</span>
                  <ICONleft theme="outline" size="18" v-if="showLabels" />
                  <right theme="outline" size="18" v-else />
                </div>
                <template v-if="showLabels">
                  <span
                    class="badge badge-neutral badge-md rounded-lg h-8 whitespace-nowrap"
                    v-for="(label, index) in problem.Label.split(/;| /)"
                    :key="index"
                    v-if="problem.Label != ''"
                  >
                    {{ label }}
                ...
                >
                  <button
                    ...
                    "
                  >
                    <ICONdata theme="outline" size="16" />
                    <span class="whitespace-nowrap font-normal">数据编辑</span>
                  </button>
                  <button
                    class="btn btn-sm btn-neutral"
                    @click="
                  ...
                  >
                    ...
                    <span class="whitespace-nowrap font-normal">题目编辑</span>
                  </button>
                </template>
              </div>
            </Card>
          ...
          <Col class="gap-0">
            <div>
              <ul
                class="menu bg-white flex flex-row rounded-box Border shadow-lg text-base font-bold rounded-b-none w-full"
              >
                <li>
                  <a
                    @click="$router.replace({ name: 'ProblemDescription' })"
                    :class="{
                      'btn-active': route.name == 'ProblemDescription',
                    }"
                  >
                ...
                <li>
                  <a
                    @click="$router.replace({ name: 'ProblemRecords' })"
                ...
                <li>
                  <a
                    @click="$router.replace({ name: 'ProblemDiscussions' })"
                    :class="{
                      'btn-active': route.name == 'ProblemDiscussions',
                    }"
                  >
                    <topic theme="outline" size="18" />
                    讨论
                    <div class="badge badge-neutral badge-md">
                      {{ problem.SolutionNumber }}
                    </div>
                  </a>
                </li>
              </ul>
      ...
      <div class="">
        ...
        >
          <path
            ...
            stroke-width="4"
            d="M12 3v.01M12 12v.01M12 21v.01"
          />
        </svg>
      </div>
    </div>
...
import CMEditor from "@/components/Main/Editor.vue";
...
let problemList = reactive<ProblemListType>({
  ...
  get() {
    ...
    _getProblemList({}, this.LID).then((data: any) => {
      this.Title = data.Title;
      this.BeginTime = data.BeginTime;
      this.EndTime = data.EndTime;
      this.Duration = ConvertTools.TimeInterval(this.BeginTime, this.EndTime);
  ...
  getProblemListUserInfo() {
    ...
    _getProblemListUserInfo(params).then((data: any) => {
...
function refreshProblemStatus() {
  ...
  if (problemList.LID && problemList.LID != undefined)
    problemList.getProblemListUserInfo();
  push.success({
    title: "已刷新题目状态",
  });
}
...
onMounted(() => {
  ...
  if (problemList.LID && userDataStore.isLogin)
    problemList.getProblemListUserInfo();
});
