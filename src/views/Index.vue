<template>
  <MainContainer>
    ...
    <Card
      class="p-6"
      v-if="groupTasksList.groupTasksList.length == 0"
    ...
    <Card class="p-6 overflow-auto h-[330px]" v-else style="text-align: center">
      ...
      <VueDraggable
        ref="el"
        v-model="groupTasksList.groupTasksList"
        target=".sort-target"
        :animation="200"
        class="select-none"
      >
        <table class="table table-zebra text-center">
          ...
          <tbody class="sort-target">
            <tr
              v-for="(item, index) in groupTasksList.groupTasksList"
              :key="item.ID"
            ...
            >
              ...
              <td class="flex justify-center space-x-2">
                <check
                  v-if="isFinished(item.ID) && isQuestion(item.type)"
                  theme="multi-color"
                  size="24"
                  fill="#7ed321"
                />
                <close
                  v-else-if="!isFinished(item.ID) && isQuestion(item.type)"
                  theme="outline"
                  size="24"
                  fill="#d0021b"
                />
    ...
    <Row direction="row">
      <!-- 轮播图 -->
      <Card class="overflow-hidden w-full h-[440px]">
        <div class="carousel w-full h-full">
          <div
            class="carousel-item relative w-full h-auto"
            v-for="(item, index) in banners.banners"
            :key="index"
            :id="`slide${index}`"
          >
            <img
              :src="'data:image/*;base64,' + item.ObjectData"
              class="w-full"
              alt="首页横幅"
            />
            <div
            ...
            >
              <a
                :href="`#slide${(index - 1 + banners.Count) % banners.Count}`"
                class="btn btn-circle"
                >❮</a
              >
              <a
                :href="`#slide${(index + 1) % banners.Count}`"
                class="btn btn-circle"
                >❯</a
              >
            </div>
      ...
      <!-- 更新日志 -->
      <Card class="w-full overflow-auto p-6 md:w-full h-[440px]">
        <Col direction="column">
          <span class="text-xl font-bold"> 版本更新日志 </span>
          <div>
            <div v-for="item in updateLogs.updateLogs" :key="item.ID">
              <div class="pb-4">
                <div class="flex items-center space-x-2">
                  ...
                  <span
                    v-if="item.Title.split('Version=').length > 1"
                ...
                <div class="-mx-4 overflow-hidden">
                  <MdPreview
                    :editorId="'updateLogs' + item.ID.toString()"
...
let group = ref({
  ...
  get() {
    ...
    _GetGroupList(params).then((data: any) => {
            ...
            if (group.value.groupList[x].GroupTask != "") {
              ...
              if (arr.length > 0) {
                ...
                for (let i = 0; i < taskLIDList.value.length; i++) {
                  _getProblemList({}, taskLIDList.value[i]).then(
                    (data: any) => {
                      groupTasksList.value.groupTasksList.push({
                        type: 3,
...
const getTrace = () => {
  ...
  for (let i = 0; i < group.value.groupList.length; i++) {
    _GetTraceGroup({ UIDs: [] }, group.value.groupList[i].GID + "").then(
      (data: any) => {
        user.value.CIDTrace =
          user.value.CIDTrace.concat(
            data.Trace[user.value.UID].CIDTrace || []
          ) || [];
        user.value.PIDTrace =
          user.value.PIDTrace.concat(
            data.Trace[user.value.UID].PIDTrace || []
          ) || [];
        user.value.LIDTrace =
          user.value.LIDTrace.concat(
            data.Trace[user.value.UID].LIDTrace || []
          ) || [];
