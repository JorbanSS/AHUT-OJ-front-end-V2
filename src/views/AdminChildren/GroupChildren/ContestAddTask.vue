<template>
      ...
      <div class="text-2xl px-6 pt-6">
        <div class="join">
          ...
          <button class="btn join-item btn-neutral" @click.stop="$router.push({
            ...
            })" style="margin-left: 10px; border-radius: 0px;">复制题单</button>
        </div>
      </div>
      <div class="mb-4"></div>
  ...
  import { ConvertTools, generatePassword, generateRandomString } from '@/utils/globalFunctions';
  ...
  onMounted(() => {
    ...
    problemList.value.LID = +route.params.LID;
    // console.log(+route.params.LID);
    if(+route.params.LID!=0)problemList.value.get();
  })
