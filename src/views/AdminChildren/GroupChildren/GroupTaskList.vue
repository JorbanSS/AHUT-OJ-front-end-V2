<template>
      ...
      <VueDraggable ref="el" v-model="taskList" target=".sort-target" :animation="200" class="select-none">
        <table class="table table-zebra text-center">
          ...
          <tbody class="sort-target">
            <tr v-for="(item, index) in taskList" :key="item.ID" class="cursor-pointer" v-auto-animate>
              <th class="w-32">
                {{ index + 1 }}
              </th>
              <th class="w-48">
  ...
  import { _GetGroup ,_EditGroup,_AddUserToGroup,_DelUserFromGroup,_TraceGroup} from '@/apis/group'; 
