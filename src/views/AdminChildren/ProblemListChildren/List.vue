<template>
  ...
  <div class="m-6 flex justify-center space-x-2">
    <ul class="menu rounded-box bg-white lg:menu-horizontal Border">
      ...
      <li>
        <div
          ...
          @click="$router.push({ name: 'AddProblemList' })"
        >
    ...
    <Confirm :action="problemLists.delete">
  ...
  <div class="bg-white card shadow-lg Border mx-auto max-w-5xl">
    <table class="table table-zebra text-center">
      ...
      <tbody>
        <tr
          v-for="(item, index) in problemLists.problemLists"
          :key="item.PID"
...
function switchSelectedStatus(index: number) {
  problemLists.problemLists[index].Selected =
    !problemLists.problemLists[index].Selected;
}
...
let problemLists = reactive<ProblemListsType>({
  ...
  delete(LID?: number) {
    ...
    _deleteProblemLists(params).then(() => {
      problemLists.get();
