<template>
    ...
    <ul class="menu rounded-box bg-white lg:menu-horizontal Border">
      <li>
        <label
          ...
          @click="$router.push({ name: 'ProblemsList' })"
        >
  ...
  >
    <div class="flex space-x-2">
      ...
      >
        ...
        <option
          v-for="item in problemOriginOptions"
          :key="item.value"
          :value="item.value"
          disabled
        >
      ...
      <label
        class="input input-bordered flex items-center gap-2 w-72"
  ...
  >
    ...
    >
      ...
      <option
        v-for="item in problemContentOptions"
        :key="item.value"
        :value="item.value"
      >
        {{ item.label }}
    ...
    <div v-else-if="problem.ContentType == 2" class="space-x-4">
      ...
      />
      <button class="btn btn-neutral" @click="problemPdf.uploadProblemPdf()">
        上传题面 PDF
      </button>
      <p class="mt-2">
        请与 PDF 加密配套使用，建议对打印和复制同时加密，密码请使用强密码
      </p>
      <p class="mt-2">
        例如：
        <a
          href="https://www.cleverpdf.com/cn/encrypt-pdf"
          target="_blank"
          class="text-blue-500"
          >https://www.cleverpdf.com/cn/encrypt-pdf
        </a>
      </p>
    </div>
  </div>
...
let problem = reactive<ProblemType>({
  ...
  get() {
    _getProblem({}, problem.PID).then((data: any) => {
  ...
  edit() {
    ...
    } else if (problem.ContentType == 1) {
      params = {
        Description: problem.Description,
      };
    }
    _editProblem(params).then(() => {
      push.success({
        title: "编辑成功",
...
function uploadProblemImage(files: any) {
  ...
  ImageUtils.compress(imageUpload.image).then((res: any) => {
    imageUpload.blob = res;
    OssUtils.uploadProblemImage(res).then((data: any) => {
...
let problemPdf = reactive<FileUploadType>({
  ...
  uploadProblemPdf() {
    ...
    OssUtils.uploadObject(this.file, "problem-pdfs", "").then((data: any) => {
      if (this.file == null) return;
      push.success({
        title: "上传成功",
        message: `文件 ${this.file.name} 大小为 ${Math.round(
          this.file.size / 1024
        )} KB`,
      });
